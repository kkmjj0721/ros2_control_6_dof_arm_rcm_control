...
class RCMGamepadController(Node):
    def __init__(self):
        ...
        self.btn_idx_a = 0
        self.btn_idx_b = 1
        self.btn_idx_x = 3  # 修改：你的X键对应的索引是3
        self.btn_idx_y = 4  # 修改：预测你的Y键对应的索引是4
        ...
        self.get_logger().info("✅ RCM 手柄解算节点已启动！")
        self.get_logger().info("按键提示: [X]标定/重力补偿 | [Y]柔顺阻抗 | [B]RCM遥操作 | [A]指令清零归位")
    ...
    def joy_callback(self, msg):
        ...
        self.get_logger().info(f"[状态] 模式: {self.current_mode} | Pitch: {self.pitch:.3f} | Yaw: {self.yaw:.3f} | Insert: {self.insertion:.3f}")

def main(args=None):
    rclpy.init(args=args)
    node = RCMGamepadController()
    try:
        rclpy.spin(node)
